...
import SearchComics from "./ComicSearch";
  ...
  if (loading) {
    ...
    return (
      <div>
        ...
        <br />
        <Button>
          <Link className="showlink"  to={`/comics/page/${parseInt(pagevalue) - 1 < 0 ? 0 : parseInt(pagevalue) - 1}`}>
            Previous
          </Link>
        </Button>
        &nbsp;
        ...
        &nbsp;
        <Button>
          <Link className="showlink" to={`/comics/page/${parseInt(pagevalue) + 1 > 991 ? 991 : parseInt(pagevalue)+ 1}`}>Next</Link>
        </Button>
      </div>
    );
  }
